...
class EmployeeReportPdf implements FromCollection, WithHeadings, WithEvents, ShouldAutoSize, WithCustomStartCell, WithTitle,WithColumnFormatting
            ...
            foreach ($registers as $item) {
                ...
                if (in_array('personalInformation.code', $allkeys)){
                   // array_push($values,  $item->code);
                    $values['personalInformation.code'] = $item->code;
                } 
                if (in_array('personalInformation.name', $allkeys)){
                    $values['personalInformation.name'] = $item->name;
                } 

                
                if (in_array('personalInformation.lastname', $allkeys)){
                    $values['personalInformation.lastname'] = $item->lastname;
                }
                if ((in_array('personalInformation.second_lastname', $allkeys))) {
                    $values['personalInformation.second_lastname'] = $item->second_lastname;
                }
                if ((in_array('personalInformation.rfc', $allkeys))) {
                    $values['personalInformation.rfc'] = $item->rfc;
                }
                if ((in_array('personalInformation.curp', $allkeys))) {
                    $values['personalInformation.curp'] = $item->curp;
                }
                if ((in_array('personalInformation.nss', $allkeys))) {
                    $values['personalInformation.nss'] = $item->nss;
                }
                if ((in_array('personalInformation.cat_gender_id', $allkeys))) {
                    $values['personalInformation.cat_gender_id'] = $item->gender->name;
                }
                if ((in_array('personalInformation.cat_marital_status_id', $allkeys))) {
                    $values['personalInformation.cat_marital_status_id'] = $item->maritalStatus->name;
                }
                if ((in_array('personalInformation.cat_country_id', $allkeys))) {
                    $values['personalInformation.cat_country_id'] = $item->country->name;
                }
                if ((in_array('personalInformation.cat_state_id', $allkeys))) {
                    $values['personalInformation.cat_state_id'] = $item->state->name;
                }
                if ((in_array('personalInformation.birthdate', $allkeys))) {
                    $values['personalInformation.birthdate'] = $item->birthdate;
                }
                if ((in_array('personalInformation.cat_educational_level_id', $allkeys))) {
                    $values['personalInformation.cat_educational_level_id'] = $item->educationLevel->name;
                }
                if ((in_array('personalInformation.email', $allkeys))) {
                    $values['personalInformation.email'] = $item->email;
                }
                if ((in_array('personalInformation.telephone', $allkeys))) {
                    $values['personalInformation.telephone'] = $item->telephone;
                }
                if ((in_array('personalInformation.cellphone', $allkeys))) {
                    $values['personalInformation.cellphone'] = $item->cellphone;
                }
              
                //personalEmergency
                if (in_array('personalEmergency.contact_name', $allkeys)) {
                     $values['personalEmergency.contact_name'] = $item->contact->name;
                }
                if (in_array('personalEmergency.contact_phone', $allkeys)){
                    $values['personalEmergency.contact_phone'] = $item->contact->phone;
                } 
                ...
                if(isset($allkeys) && count($allkeys) > 0 && $this->checkExistsKey('personalLabor') && isset($item->employments)){
                    ...
                    foreach ($item->employments as $employment) {
                        ...
                        if (in_array('personalLabor.cat_contract_type_id', $allkeys)) {
                            $tmp .= "Tipo de contratación: {$employment->contract_type->name} ";  
                        }
