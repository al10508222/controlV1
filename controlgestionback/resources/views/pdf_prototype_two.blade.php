<table style="width:100%;" class="textoFormato">
    <tr>
        <td>
...
<br>
<table class="tb-border textoFormato" style="width:100%">
    <tr>
        <th>No. Trabajador:</th>
    ...
    <tr>
        ...
        <td>{{ $data->curp }} &nbsp;&nbsp;</td>
        <th>No. Nómina:</th>
        <td>{{ $data->processed_payroll_id }}</td>
    </tr>
    <tr>
        <th>RFC: </th>
        <td>{{ $data->rfc }} &nbsp;&nbsp;</td>
        <th>Periodo:</th>
        <td>{{ $data->payment_period_start }} al {{  $data->payment_period_end }}</td>
    </tr>
</table>
<table style="table-layout: fixed; word-break: break-all;" class="textoFormato">
    <thead>
        <tr width="100%">
            <th class="bg-carmin-two">
    ...
    <tbody>
        <tr>
            <td>
                <table width="100%" class="textoFormato">
                    <tbody class="">
                        @php( $row = 0 )
                        @foreach($perceptions as $perception)
                        {{ $row++ }}
                        <tr class="bg-{{ $row % 2 == 0 ? "dark-grey" : "light-grey" }}">
                            <td width=20%>{{ $perception->code }}</td>
                            <td width=50%>{{ $perception->name }}</td>
                            <td width=30% class="text-right">{{ number_format($perception->amount,2, '.', ',') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <td>
                <table width="100%" class="textoFormato">
                    <tbody class="">
                        ...
                        {{ $row++ }}
                        <tr class="bg-{{ $row % 2 == 0 ? "dark-grey" : "light-grey" }}">
                            <td width=20%>{{ $deduction->code }}</td>
                            <td width=50%>{{ $deduction->name }}</td>
                            <td width=30% class="text-right">{{ number_format($deduction->amount,2, '.', ',') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
...
<br>
<br>
<hr style="border: 1px solid {{ $setting['primary'] }}">
<table class="table-small-padding textoFormato" style="table-layout: fixed; word-break: break-all;">
    <colgroup>
        <col width="10%" />
        <col width="10%" />
        <col width="10%" />
    ...
    <tr>
        ...
        <td class="text-right">{{ number_format($data->total_perceptions,2, '.', ',') }}</td>
        <td></td>
    ...
    <tr>
        <td></td>
        <td>Neto Pagado</td>
        <td class="text-right">{{ number_format($data->total_perceptions - $data->total_deductions,2, '.', ',') }}</td>
    </tr>
    <tr>
        <td></td>
        <td>Total en Efectivo</td>
        <td class="text-right">{{ $totalEfectivo }}</td>
        <td></td>
        <td colspan="3" class="text-right"> Firma: ____________________________</td>
    </tr>
</table>
<div class="bg-carmin-two textoFormato">
    <center><b> COMPROBANTE FISCAL DIGITAL POR INTERNET </b></center>
</div>
<table class="table-small-padding text-small textoFormato" style="width:100%">
    <tr>
        <th>Folio Fiscal:</th>
...
<table class="text-small table-small-padding textoFormato" style="width:100%;">
    <tr>
        <td class="text-center">
            <img src="https://imgssl.constantcontact.com/kb/qr-code-standard-example.png" height="120px">
        </td>
        <td>
            <table class="textoFormato">
                <tr>
                ...
                <tr>
                    <td colspan="2"><hr style="border: 1px solid {{ $setting['primary'] }}"></td>
                </tr>
                <tr>
...
<br>
<div class="text-small textoFormato">
