            ...
            <q-card-section>
                <q-form ref="form"  @submit.prevent="() => {}">
                    ...
                    <q-card-actions align="right">
                        ...
                        <q-btn flat @click="update()" label="Actualizar" color="primary" />
                    </q-card-actions>
                </q-form>
            </q-card-section>
        </div>
  ...
  mounted() {
    ...
    this.$store
      ...
      .then(() => {
        ...
        if (id !== undefined) {
          DataService.edit(this.$route.params.employee_id, id).then((data) => {
            this.incident = data;
          })
          this.$q.loading.hide()
        }
    ...
    update() {
      ...
      this.$refs.form.validate().then((valid) => {
        if (valid && this.incident.days.length > 0) {
          ...
          const { id } = this.$route.params;
          DataService.update(formData, id).then((res) => {
            if (res.success) {
              notifySuccess();
              this.$router.back()
            } else {
