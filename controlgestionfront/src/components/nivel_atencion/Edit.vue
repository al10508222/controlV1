<template>
<div>
  <q-form ref="NivelAtencionForm" @submit.prevent="() => {}">
    <q-card style="width: 100%">
      <q-item>
        ...
        <q-item-section>
          <q-item-label>Editar Nivel de Atención</q-item-label>
        </q-item-section>
      </q-item>
      <q-separator />
      <div class="q-pa-sm">
        <q-card-section>
          <div class="row q-col-gutter-sm">
            <div class="col-xs-3 col-sm-3 col-md-3">
              <q-input v-model="form.NIVELATENCIONID" square outlined label="ID" :disable="true"/>
            </div>
            <div class="col-xs-3 col-sm-3 col-md-3">
...
export default {
  ...
  created() {
    const catalogsConfiguration = { profiles: true, nivel_atencion: true };
  ...
  methods: {
    update() {
      ...
      NivelAtencionServices.update(form, id).then((data) => {
        console.log(data)
        notifySuccess();
        this.$router.push('../../nivel_atencion');
      }).catch((err) => {
        notifyError(err)
      })
    },
