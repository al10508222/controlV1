      ...
      <q-form ref="form" @submit.prevent="() => {}">
        <div class="row q-col-gutter-sm">
          ...
          <div class="col-xs-4 col-sm-4 col-md-4">
            <q-input
              :rules="[$rules.required($i18n.t('requiredInput'))]"
              v-model="form.INSTITUCIONID"
              square
              outlined
              label="INSTITUCION ID" type="text"/>
          </div>
          <div class="col-xs-4 col-sm-4 col-md-4">
            <q-input
              :rules="[$rules.required($i18n.t('requiredInput'))]"
              v-model="form.INSTITUCIONNOMBRE"
              square
              outlined
              label="INSTITUCION NOMBRE" type="text"/>
          </div>
          <div class="col-xs-4 col-sm-4 col-md-4">
            <q-input
              :rules="[$rules.required($i18n.t('requiredInput'))]"
              v-model="form.INSTITUCIONIDCORTO"
              square
              outlined
              label="INSTITUCION ID CORTO" type="text"/>
          </div>
        </div>
      </q-form>
    </q-card-section>
...
export default {
  ...
  created() {
    const catalogsConfiguration = { profiles: true, institucion: true };
    this.$q.loading.show();
    ...
    store() {
      this.$refs.form.validate().then((valid) => {
        if (valid) {
          ...
          InstitucionesServices.store(form).then(() => {
            notifySuccess();
