<template>
  <q-card style="width: 700px; max-width: 80vw;">
    ...
    <q-card-section>
      <p> El Excel entrara en proceso de validación para la carga masiva </p>
...
export default {
  ...
  methods: {
    upload() {
      ...
      this.$q.loading.show();
      UploadFileService.uploadContractExcel(formData).then((file) => {
        ...
        this.$root.$emit('refresh-contracts');
        this.$q.loading.hide();
      }).catch((err) => {
        console.log(err)
        this.$q.loading.hide();
