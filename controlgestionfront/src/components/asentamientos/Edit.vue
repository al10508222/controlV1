  ...
  <q-form ref="estratoForm" @submit.prevent="() => {}">
    <q-card style="width: 100%">
      <q-item>
        ...
        <q-item-section>
          <q-item-label>Editar Asentamientos</q-item-label>
        </q-item-section>
      </q-item>
      <q-separator />
      <div class="q-pa-sm">
        <q-card-section>
          <div class="row q-col-gutter-sm">
            <div class="col-xs-3 col-sm-3 col-md-3">
              <q-input v-model="form.TIPOASENTAMIENTOID" square outlined label="ID" :disable="true"/>
            </div>
            <div class="col-xs-3 col-sm-3 col-md-3">
              <q-input v-model="form.TIPOASENTAMIENTONOMBRE" square outlined label="Nombre Asentamiento"/>
            </div>
          </div>
        </q-card-section>
...
export default {
  ...
  methods: {
    update() {
      ...
      AsentamientosServices.update(form, id).then((data) => {
        console.log(data)
        notifySuccess();
        this.$router.push('../../asentamientos');
      }).catch((err) => {
        notifyError(err)
      })
    },
