  ...
  <q-form ref="tipologiaForm" @submit.prevent="() => {}">
    <q-card style="width: 100%">
      <q-item>
        ...
        <q-item-section>
          <q-item-label>Editar Tipologia</q-item-label>
        </q-item-section>
      </q-item>
      <q-separator />
      <div class="q-pa-sm">
        <q-card-section>
          <div class="row q-col-gutter-sm">
            <div class="col-xs-3 col-sm-3 col-md-3">
              <q-input v-model="form.TIPOLOGIAID" square outlined label="ID" :disable="true"/>
            </div>
...
export default {
  ...
  methods: {
    update() {
      ...
      TipologiaServices.update(form, id).then((data) => {
        console.log(data)
        notifySuccess();
        this.$router.push('../../tipologia');
      }).catch((err) => {
        notifyError(err)
      })
    },
